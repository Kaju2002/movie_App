...
// Contexts
import { useMovieData } from '../context/MovieDataContext'; // Adjust path if necessary
import { useThemeContext } from '../context/ThemeContext'; // Adjust path if necessary
...
`;
// const slideInFromLeft = keyframes`/* ... */`; // Define if using different title animation
// const slideInFromRight = keyframes`/* ... */`; // Define if using different title animation
...
const Navbar = () => {
        ...
        {drawerNavLinks.map((item) => (
          <ListItem key={item.text} disablePadding sx={{mb: 0.5}}>
            <ListItemButton
                ...
                style={({ isActive }) => isActive ? activeDrawerStyle : defaultDrawerStyle}
                sx={{ py: 1.2, px: 2, borderRadius: 1, '&:hover': { bgcolor: alpha(theme.palette.action.hover, 0.06) } }}
            >
        ...
        {isSignedIn && (
            <ListItem disablePadding sx={{mb: 0.5}}>
                <ListItemButton
                    ...
                    sx={{ py: 1.2, px: 2, borderRadius: 1, color: 'text.primary', '&:hover': { bgcolor: alpha(theme.palette.action.hover, 0.06) } }}
                >
                    <ListItemIcon sx={{ minWidth: 'auto', mr: 2, color: 'inherit' }}><LogoutIcon /></ListItemIcon>
                    <ListItemText primary="Logout" primaryTypographyProps={{ fontWeight: 'inherit', fontSize: '0.95rem' }} />
                </ListItemButton>
            </ListItem>
        )}
      </List>
  ...
  const desktopNavLinkSx = (currentTheme) => ({ // Changed param name to avoid conflict
    color: currentTheme.palette.text.secondary,
    fontWeight: 500, mx: 0.25, textTransform: 'capitalize', fontSize: '0.9rem',
    letterSpacing: '0.025em', borderRadius: '16px', px: 1.75, py: 0.75,
    transition: currentTheme.transitions.create(['color', 'background-color', 'transform']),
    position: 'relative', overflow: 'hidden',
    '&::after': { content: '""', position: 'absolute', bottom: '4px', left: '50%', transform: 'translateX(-50%) scaleX(0)', width: '0%', height: '2px', backgroundColor: currentTheme.palette.primary.main, transition: currentTheme.transitions.create(['width', 'transform'])},
    '&:hover': { color: currentTheme.palette.text.primary, bgcolor: alpha(currentTheme.palette.action.hover, 0.08), transform: 'translateY(-1px)'},
    '&.active': { color: currentTheme.palette.primary.main, fontWeight: '600', '&::after': { width: '60%', transform: 'translateX(-50%) scaleX(1)' }}
  });
      ...
      >
        <Toolbar sx={{ justifyContent: 'space-between', minHeight: { xs: 56, sm: 64 } }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', flexShrink: 0 }}>
            <IconButton color="inherit" aria-label="open drawer" edge="start" onClick={handleDrawerToggle} sx={{ mr: { xs: 0.5, sm: 1.5 }, display: { md: 'none' } }} > <MenuIcon /> </IconButton>
          ...
          {/* Center: Search Bar */}
          <Box component="form" onSubmit={handleSearchSubmit} sx={(currentTheme) => ({ // Use currentTheme from useTheme()
              display: { xs: 'none', md: 'flex' }, alignItems: 'center', position: 'relative',
              borderRadius: '25px', backgroundColor: alpha(currentTheme.palette.action.selected, 0.1),
              border: `1px solid ${alpha(currentTheme.palette.divider, 0.5)}`,
              transition: currentTheme.transitions.create(['background-color', 'border-color', 'box-shadow']),
              '&:hover': { backgroundColor: alpha(currentTheme.palette.action.selected, 0.15), borderColor: currentTheme.palette.divider, },
              '&:focus-within': { backgroundColor: alpha(currentTheme.palette.action.selected, 0.2), borderColor: currentTheme.palette.primary.main, boxShadow: `0 0 0 2px ${alpha(currentTheme.palette.primary.main, 0.3)}`},
              mx: 2, flexGrow: 1, maxWidth: '550px',
            })}
          >
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', flexShrink: 0 }}>
            ...
            <Box sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center', mr: 0.5 }}>
              {desktopBaseNavLinks.map((item) => ( // Use desktopBaseNavLinks
                <Button key={item.text} component={RouterNavLink} to={item.path} end={item.path === '/'} sx={desktopNavLinkSx(theme)} >
                  {item.text}
                </Button>
              ))}
              {isSignedIn && (
                <Button component={RouterNavLink} to="/favorites" sx={desktopNavLinkSx(theme)} >
                  Favorites
                </Button>
              )}
            </Box>
            ...
            {isSignedIn ? (
              <>
                <IconButton onClick={handleOpenUserMenu} sx={{ p: 0, ml: 1.5, width: 36, height: 36 }}>
                  <Avatar alt={user?.fullName || user?.primaryEmailAddress?.emailAddress || 'User'} src={user?.imageUrl} sx={{ width: 32, height: 32, border: `2px solid ${theme.palette.primary.light}` }}>
                    {!user?.imageUrl && <AccountCircleIcon sx={{fontSize: '2rem'}}/>}
                  </Avatar>
                </IconButton>
                <Menu
                  anchorEl={anchorElUser} open={Boolean(anchorElUser)} onClose={handleCloseUserMenu}
                  PaperProps={{ elevation: 0, sx: { overflow: 'visible', filter: 'drop-shadow(0px 2px 8px rgba(0,0,0,0.2))', mt: 1.5, bgcolor: 'background.paper', '& .MuiAvatar-root': { width: 32, height: 32, ml: -0.5, mr: 1, }, '&::before': { content: '""', display: 'block', position: 'absolute', top: 0, right: 14, width: 10, height: 10, bgcolor: 'background.paper', transform: 'translateY(-50%) rotate(45deg)', zIndex: 0, }, }, }}
                  transformOrigin={{ horizontal: 'right', vertical: 'top' }}
                  anchorOrigin={{ horizontal: 'right', vertical: 'bottom' }}
                >
                  <MenuItem onClick={() => { handleCloseUserMenu(); navigate('/user-profile'); }}> <ListItemIcon><PersonOutlineIcon fontSize="small" /></ListItemIcon> Profile </MenuItem>
                  <Divider sx={{my: 0.5}}/>
                  <MenuItem onClick={async () => { handleCloseUserMenu(); await signOut(() => navigate('/')); }}> <ListItemIcon><LogoutIcon fontSize="small" /></ListItemIcon> Logout </MenuItem>
                </Menu>
              </>
            ) : (
              <Button component={RouterNavLink} to="/sign-in" sx={desktopNavLinkSx(theme)} >
                Login
              </Button>
            )}
