...
export const MovieDataProvider = ({ children }) => {
  ...
  const [favorites, setFavorites] = useState(() =>
    JSON.parse(localStorage.getItem("movieExplorerFavorites") || "[]")
  );
  ...
  // --- Last Searched Term ---
  const [lastSearch, setLastSearchState] = useState(
    () => localStorage.getItem("movieExplorerLastSearch") || ""
  );
  ...
  const [currentAllMoviesSortBy, setCurrentAllMoviesSortBy] =
  ...
  useEffect(() => {
    const fetchInitialAppData = async () => {
      try {
        ...
        const [featRes, trendRes, popRes, topRes, upcRes] =
        ...
        if (trendRes.status === "fulfilled")
          setTrendingMoviesState((trendRes.value.results || []).slice(0, 12));
        ...
        if (
          [featRes, trendRes, popRes, topRes, upcRes].some(
            (r) => r.status === "rejected"
          )
        ) {
  ...
  const fetchOrLoadMoreAllMovies = useCallback(
    async (sortBy = currentAllMoviesSortBy, filters = {}, reset = false) => {
      ...
      if (
        !reset &&
        pageToFetch > allMoviesTotalPages &&
        allMoviesTotalPages !== 0 &&
        allMoviesTotalPages !== 1
      ) {
  ...
  const value = {
    ...
    searchError,
    searchCurrentPage,
    searchTotalPages, // Expose for pagination
