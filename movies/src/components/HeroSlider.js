...
import { getImageUrl, getMovieVideos, findTrailerKey } from '../services/tmdbService'; // Ensure path is correct
import TrailerModal from './TrailerModal'; // Import the modal component

// Define your Movie type (example, adjust as per your actual data structure from TMDb)
// interface Movie {
//   id: number | string;
//   title: string;
...
const HeroSlider = ({ movies, isLoading }) => {
  ...
  if (isLoading) {
    return <Skeleton variant="rectangular" animation="wave" width="100%" height={{ xs: '50vh', sm: '60vh', md: '70vh' }} />;
  }

  if (!movies || movies.length === 0) {
    return (
      <Box sx={{ height: { xs: '50vh', sm: '60vh', md: '70vh' }, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: 'grey.900' }}>
        <Typography variant="h5" color="text.secondary">No featured movies available.</Typography>
      </Box>
    );
  }

  const currentMovie = movies[currentIndex];
  const year = currentMovie?.release_date ? currentMovie.release_date.substring(0, 4) : 'N/A';
    ...
    <>
      ...
      >
        ...
        <Box sx={{
          position: 'absolute',
          inset: 0,
          backgroundImage: 'linear-gradient(to top, rgba(0,0,0,0.95) 0%, rgba(0,0,0,0.7) 25%, rgba(0,0,0,0.3) 50%, transparent 100%)',
          zIndex: 1,
        }} />
        ...
        >
          ...
          >
            {voteAverage && voteAverage > 0 && (
              <Box sx={{ bgcolor: imdbYellow, color: 'black', fontWeight: 'bold', px: 1, py: 0.25, fontSize: '0.8rem', borderRadius: '4px' }}>
                IMDb {voteAverage.toFixed(1)}
              </Box>
            )}
            <Typography variant="body2" sx={{ color: 'grey.300', fontSize: '0.9rem' }}>{year}</Typography>
          </Box>
          ...
          >
            <Button
              ...
              variant="contained"
              size="large"
              startIcon={trailerLoading ? <CircularProgress size={20} color="inherit"/> : <PlayArrowIcon />} // Show spinner when loading
              sx={{
                bgcolor: imdbYellow,
                color: 'black',
                fontWeight: 'bold',
        ...
        {movies.length > 1 && (
          <Box sx={{ /* ... styles */ }} >
              {Array.from({ length: Math.min(movies.length, 7) }).map((_, index) => (
              <IconButton
                  key={`hero-dot-${index}`}
                  size="small"
