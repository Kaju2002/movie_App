...
const SearchResultsPage = () => {
  ...
  useEffect(() => {
    ...
    if (queryFromUrl) {
      ...
      if (queryFromUrl !== contextQuery) { // Only update if different
  ...
  const genres = ['Action', 'Adventure', 'Animation', 'Comedy', 'Crime', 'Drama', 'Fantasy', 'Horror', 'Sci-Fi', 'Thriller'];
  const currentYear = new Date().getFullYear();
  const years = Array.from({ length: 50 }, (_, i) => currentYear - i);

  return (
    <Box sx={{ bgcolor: 'background.default', color: 'text.primary', minHeight: 'calc(100vh - 128px)', py: 4 }}>
      <Container maxWidth="lg">
        <Paper
          elevation={3}
          sx={{ p: { xs: 2, sm: 3 }, mb: 4, bgcolor: alpha(theme.palette.background.paper, 0.85), backdropFilter: 'blur(5px)', borderRadius: 2 }}
        >
          <Box component="form" onSubmit={handleLocalSearchSubmit} sx={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            <TextField
              fullWidth
              variant="outlined"
            ...
            {localSearchInput && (
              <IconButton
                ...
                sx={{ position: 'absolute', right: { xs: 45, sm: 50 }, top: '50%', transform: 'translateY(-50%)', color: 'text.secondary' }}
              > <ClearIcon /> </IconButton>
            )}
            <IconButton type="submit" aria-label="Search" sx={{ ml: 1, color: imdbYellow, bgcolor: alpha(imdbYellow, 0.1), '&:hover': { bgcolor: alpha(imdbYellow, 0.2)} }}>
              <SearchIconMUI />
            </IconButton>
          </Box>

          <Box sx={{ mt: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Button startIcon={<FilterListIcon />} onClick={toggleFilters} size="small" sx={{ color: 'text.secondary', textTransform: 'none' }} >
          ...
          <Collapse in={filterOpen}>
            <Box sx={{ mt: 2, pt: 2, borderTop: `1px solid ${theme.palette.divider}` }}>
                ...
                {['year', 'genre', 'rating'].map((filterType) => (
                  <Grid item xs={12} sm={4} key={filterType}>
                    <FormControl fullWidth size="small">
                      ...
                      <Select
                        name={filterType} value={filters[filterType]} label={filterType.charAt(0).toUpperCase() + filterType.slice(1)} onChange={handleFilterChange}
                      >
                        <MenuItem value=""><em>All {filterType === 'genre' ? 'Genres' : filterType.charAt(0).toUpperCase() + filterType.slice(1)}s</em></MenuItem>
                        {filterType === 'year' && years.map(y => <MenuItem key={y} value={y}>{y}</MenuItem>)}
                        {filterType === 'genre' && genres.map(g => <MenuItem key={g} value={g}>{g}</MenuItem>)}
                        {filterType === 'rating' && [9,8,7,6,5].map(r => <MenuItem key={r} value={r}>{r}+</MenuItem>)}
                      </Select>
                    </FormControl>
                  </Grid>
                ))}
              ...
              <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
                <Button onClick={resetFilters} size="small" sx={{color: 'text.secondary'}}>Reset</Button>
                <Button onClick={applyFilters} variant="contained" size="small" sx={{bgcolor: imdbYellow, color: 'black', '&:hover': {bgcolor: alpha(imdbYellow, 0.8)}}}>Apply Filters</Button>
              </Box>
            </Box>
          </Collapse>
        </Paper>
        ...
        {!isSearching && searchError && (
          <Alert severity="info" sx={{ justifyContent: 'center' }}>{searchError}</Alert>
        )}
        {!isSearching && !searchError && contextQuery && searchResults.length > 0 && (
          <>
        ...
        {!isSearching && !searchError && !contextQuery && ( // Show when no search term is active
            <Box sx={{ textAlign: 'center', py: 8, color: 'text.secondary' }}>
                <ManageSearchIcon sx={{ fontSize: 60, mb: 2 }} />
                <Typography variant="h5" gutterBottom>Find Your Next Favorite Movie</Typography>
                <Typography>Use the search bar above to discover movies.</Typography>
            </Box>
        )}
      </Container>
    </Box>
