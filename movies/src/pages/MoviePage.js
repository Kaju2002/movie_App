...
const { isSignedIn } = useUser(); // ✅ To check if user is signed in
const [loginPromptOpen, setLoginPromptOpen] = useState(false); // Snackbar control

  const handleToggleFavorite = () => {
  if (!isSignedIn) {
    setLoginPromptOpen(true); // 🔐 Show login snackbar
    return;
  }
...
const handleCloseLoginPrompt = (event, reason) => {
      ...
      try {
        ...
        if (data) {
          ...
          document.title = `${data.title} (${
            data.release_date?.substring(0, 4) || "N/A"
          }) - Movie Explorer`;
        } else {
  ...
  const writers =
    movie.credits?.crew
      ...
      .map((p) => p.name)
      .slice(0, 3)
      .join(", ") || "N/A"; // Show up to 3 writers
  const cast = movie.credits?.cast || [];
  const photos = movie.images?.backdrops?.slice(0, 8) || []; // Get up to 8 backdrops
  const similarMovies =
    movie.similar?.results || movie.recommendations?.results || []; // Use similar or recommendations

  return (
    <>
      <Box sx={{ bgcolor: "background.default", color: "text.primary", pb: 8 }}>
        {" "}
        {/* Padding bottom for mobile action bar */}
        {/* 1. Backdrop Section */}
        <Box
          sx={{
          ...
          <Grid container spacing={{ xs: 2, sm: 3, md: 4 }}>
            ...
            <Grid item xs={12} sm={4} md={3}>
              <Box sx={{ position: "sticky", top: "80px" }}>
                {" "}
                {/* Make poster sticky on scroll */}
                <Paper
                  elevation={6}
                  sx={{ aspectRatio: "2/3", overflow: "hidden", mb: 2 }}
                >
                  <img
                    src={posterUrl}
                    alt={movie.title}
          ...
          >
            <Toolbar sx={{ justifyContent: "space-around" }}>
              ...
              >
                ...
                <Typography variant="caption" sx={{ mt: 0.5, lineHeight: 1 }}>
