...
import HeroSlider from '../components/HeroSlider'; // Your MUI HeroSlider
import Carousel from '../CommonPage/Corousel';   // Your MUI Carousel
...
const HomePage = () => {
  ...
  const renderSection = (title, movies, icon, isLoading, cardSize = 'medium') => {
    if (isLoading && !movies.length) { // Show skeleton/loading only if no movies yet
        return (
            <Box sx={{ mb: 4 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    {icon}
                    <Typography variant="h5" component="h2" sx={{ ml: 1, fontWeight: 'semibold' }}>{title}</Typography>
                </Box>
                <Typography>Loading movies...</Typography> {/* Or a Carousel Skeleton */}
            </Box>
        );
    }
    ...
    return (
      <Box sx={{ mb: { xs: 4, sm: 5 } }}> {/* mb-8 */}
        <Box sx={{ display: 'flex', alignItems: 'center', mb: {xs: 2, sm: 3} /* mb-6 or mb-4 */ }}>
          {icon}
          <Typography variant="h5" component="h2" sx={{ ml: 1, fontWeight: 'semibold' }}>{title}</Typography>
        </Box>
        <Carousel
    ...
    <Box sx={{ bgcolor: 'background.default', color: 'text.primary' }}>
      ...
      <Container maxWidth="xl" sx={{ py: { xs: 3, sm: 4 } /* py-6 */ }}>
        {renderSection(
          ...
          <LocalFireDepartmentIcon sx={{ color: imdbYellow, fontSize: {xs: '1.5rem', sm: '1.75rem'} /* size 24 */}} />,
        ...
        {renderSection(
          ...
          <TrendingUpIcon sx={{ color: imdbYellow, fontSize: {xs: '1.25rem', sm: '1.5rem'} /* size 20 */}} />,
        ...
        {renderSection(
          "Top Rated",
          topRatedMovies,
          <StarBorderPurple500Icon sx={{ color: imdbYellow, fontSize: {xs: '1.25rem', sm: '1.5rem'} }} />,
        ...
        {renderSection(
          "New Releases",
          newReleases,
          <EventNoteIcon sx={{ color: imdbYellow, fontSize: {xs: '1.25rem', sm: '1.5rem'} }} />,
